...
birthweight_df_with_preds_resids <- birthweight_df |>
...
birthweight_df_with_preds_resids
```
...
``` r
model1 <- lm(bwt ~ blength + bhead + babysex + gaweeks + delwt + fincome + 
              smoken + wtgain + momage + mheight + parity, data = birthweight_df)
...
model_comparison <- tibble(
  Model = c("Model 1 (Full)", "Model 2 (Main Effects Only)", "Model 3 (Interactions)"),
...
```
    ...
    ##   Model                          AIC R_squared
    ##   <chr>                        <dbl>     <dbl>
    ## 1 Model 1 (Full)              61207.     0.706
    ## 2 Model 2 (Main Effects Only) 62771.     0.577
    ## 3 Model 3 (Interactions)      61501.     0.685

``` r
# Residual plots for each model
model1_resids <- add_predictions(birthweight_df, model1) %>%
  add_residuals(model1)

model2_resids <- add_predictions(birthweight_df, model2) %>%
  add_residuals(model2)

model3_resids <- add_predictions(birthweight_df, model3) %>%
  add_residuals(model3)

# Plot residuals for each model
ggplot(model1_resids, aes(x = pred, y = resid)) +
  geom_point() +
  geom_smooth(method = "lm", color = "lightgreen") +
  labs(title = "Residuals vs Fitted for Model 1 (Full Model)", x = "Fitted Values", y = "Residuals") +
  theme_minimal()
```
...
``` r
ggplot(model2_resids, aes(x = pred, y = resid)) +
  geom_point() +
  geom_smooth(method = "lm", color = "tomato") +
  labs(title = "Residuals vs Fitted for Model 2 (Main Effects Only)", x = "Fitted Values", y = "Residuals") +
  theme_minimal()
```

    ## `geom_smooth()` using formula = 'y ~ x'

![](p8105_hw6_iah2124_files/figure-gfm/unnamed-chunk-15-2.png)<!-- -->

``` r
ggplot(model3_resids, aes(x = pred, y = resid)) +
  geom_point() +
  geom_smooth(method = "lm", color = "orchid") +
  labs(title = "Residuals vs Fitted for Model 3 (Interactions)", x = "Fitted Values", y = "Residuals") +
  theme_minimal()
```

    ## `geom_smooth()` using formula = 'y ~ x'

![](p8105_hw6_iah2124_files/figure-gfm/unnamed-chunk-15-3.png)<!-- -->
